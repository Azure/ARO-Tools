...
func TestProcessPipelineForEV2(t *testing.T) {
	...
	pipelinePath := "../../testdata/pipeline.yaml"
	originalPipeline, err := types.NewPipelineFromFile(pipelinePath, vars)
	if err != nil {
	...
	newPipeline, newFiles, err := processPipelineForEV2(originalPipeline, pipelinePath, files, vars)
	if err != nil {
