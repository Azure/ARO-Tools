...
func TestNewDelegateChildZoneStep(t *testing.T) {
	tests := []struct {
		name     string
		input    string
		expected DelegateChildZoneStep
		err      bool
	}{
		{
			name: "TestNewDelegateChildZoneStep_ValidInput",
			input: `
name: test-zone
action: DelegateChildZone`,
			expected: *NewDelegateChildZoneStep("test-zone"),
		},
		{
			name: "TestAllOptions",
			input: `
  ...
  value: parent`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var output DelegateChildZoneStep
