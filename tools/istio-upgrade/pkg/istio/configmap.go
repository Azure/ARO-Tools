...
func CreateRevisionConfigMap(ctx context.Context, kubeClient *KubeClient, revision string) error {
	...
	existing, err := kubeClient.client.CoreV1().ConfigMaps(istioSystemNamespace).Get(ctx, cmName, metav1.GetOptions{})
	if err == nil {
		if existing.Data["mesh"] == meshData && existing.Labels["istio.io/rev"] == revision {
			return nil
		}
		...
		updated.Labels["istio.io/rev"] = revision
		updated.Data = map[string]string{"mesh": meshData}
		if _, err = kubeClient.client.CoreV1().ConfigMaps(istioSystemNamespace).Update(ctx, updated, metav1.UpdateOptions{}); err != nil {
	...
	cm := &corev1.ConfigMap{
		...
		Data: map[string]string{"mesh": meshData},
	}
	if _, err = kubeClient.client.CoreV1().ConfigMaps(istioSystemNamespace).Create(ctx, cm, metav1.CreateOptions{}); err != nil {
...
func DeleteRevisionConfigMap(ctx context.Context, kubeClient *KubeClient, revision string) error {
	cmName := revisionConfigMapName(revision)
	if err := kubeClient.client.CoreV1().ConfigMaps(istioSystemNamespace).Delete(ctx, cmName, metav1.DeleteOptions{}); err != nil {
		if apierrors.IsNotFound(err) {
			return nil
		}
